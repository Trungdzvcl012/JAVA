...
if (hoaDon == null) {
    ...
    public String thucHienThanhToan(@RequestParam("hoaDonId") Long hoaDonId,
        ...
        switch (phuongThuc.toLowerCase()) {
            case "momo" -> model.addAttribute("momoUrl", momoService.createPaymentUrl(hoaDon));
            case "vnpay" -> model.addAttribute("vnpUrl", vnpayService.createPaymentUrl(hoaDon));
            case "zalopay" -> model.addAttribute("zalopayUrl", zalopayService.createPaymentUrl(hoaDon));
            default -> model.addAttribute("success", "Thanh toán thành công bằng phương thức: " + phuongThuc.toUpperCase());
        }
